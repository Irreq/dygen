...
class KernelGenerator(object):
    ...
    def kernel_generator(self, localkernel):
        ...
        for k in range(len(localkernel)):
            ...
            distribution = self.kernel_normal_dist(
                localkernel[k][0], localkernel[k][1], self.size)
        ...
        if self.debug:
            ...
            print('elements : {}\nmean : {}\nmax : {}\nmin : {}'.format(
                len(self.data), np.mean(np.array(self.data)), max(self.data), min(self.data)))

            plt.hist(np.array(self.data), bins=np.linspace(
                0, 1, num=100).tolist())
            plt.show()
    ...
    def random_kernels(self, n_kernels):
        ...
        self.bias['normal_distribution'] = self.kernel_error_catcher(
            self.bias['normal_distribution'])
    ...
    def setwindow(self, lower, upper, kernel_id):
        ...
        if type(kernel_id) == str:
            ...
            try:
                ...
                print(e)
                print("Window {} < x < {} could not be set, due to:".format(
                    lower, upper))
                return
        ...
        data = [i*(upper-lower)+lower for i in data[0]]
    ...
    def fastgen(self, distribution):
        ...
        for i, value in enumerate(distribution):
            ...
            data = [i*window+np.random.choice(window_distribution)
...
res = kg.fastgen([0.66, 0.206, 0.128, 0.005, 1])


plt.hist(res, bins=np.linspace(0, 1, 10))
plt.show()
